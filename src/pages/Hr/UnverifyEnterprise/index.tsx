...
import {Alert, Button, Card, Col, Descriptions, List, notification, Popconfirm, Row, Skeleton} from "antd";
...
export default class  Index extends React.Component<any, any>{
  auditEnterprise= async  (item)=>{
    this.setState({isLoading:true})
    await  HrStore.passEnterpriseInfo(item.enterpriseId)
    this.setState({isLoading:false})
  }

  deleteEnterprise = async  (item)=>{
    this.setState({isLoading:true})
    await HrStore.fallbackEnterpriseInfo(item.enterpriseId)
    this.setState({isLoading:false})
  }
               ...
               type="warning"  closable={true} />
        {
          this.state.isLoading ?
            ...
            <List
              ...
              renderItem={ (item :any ) =>{
                return (
                  <ProCard hoverable={true} style={{marginBottom:'40px'}} layout="default" bordered colSpan={22}  >
                    <Row gutter={[16, 16]} align={'top'} justify={'center'} wrap={true}>
                      ...
                      <Col span={24}>
                        <Descriptions  style={{marginTop:'40px'}}
                                       ...
                                       column={2}
                        >
                          <Descriptions.Item label="企业法人姓名" span={4}>{item.enterpriseCorperationName}</Descriptions.Item>
                          <Descriptions.Item label="企业法人手机号" span={4}>{item.enterpriseCorperationPhoneNumber}</Descriptions.Item>
                        </Descriptions>
                    ...
                    <Row gutter={[16, 16]} align={'top'} style={{marginTop:'30px'}} justify={'space-around'} wrap={true}>
                      <Popconfirm placement="top" title={'审核确定?'} onConfirm={ ()=>{
                        this.auditEnterprise(item)
                      }}  okText="是" cancelText="否">
                        <Button type={"default"} shape={"round"} style={{width:'40%'}}>审核通过该公司</Button>
                      </Popconfirm>

                      <Popconfirm placement="top" title={'审核确定?'} onConfirm={()=>{
                        this.deleteEnterprise(item)
                      }} okText="是" cancelText="否">
